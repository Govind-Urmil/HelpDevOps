...
if(pkg.version!==release.version||release.version!=='0.5.0'||release.ep!=='EP-005') throw new Error('Release version metadata is inconsistent.');
...
for(const specFile of ['EP-002-SPEC.md','EP-003-SPEC.md','EP-004-SPEC.md','EP-005-SPEC.md']){const spec=fs.readFileSync(path.join(root,'docs',specFile),'utf8');if(spec.length<1000||!spec.includes('Acceptance Criteria'))throw new Error(`Authoritative specification is incomplete: ${specFile}`);}
...
for(const file of currentUiFiles){const source=fs.readFileSync(path.join(root,file),'utf8');if(/EP-00[1-4]/.test(source))throw new Error(`${file}: stale current-release EP wording`);}
...
for(const required of ['docs/LOCAL-DATA-AND-PRIVACY-MODEL.md','docs/WORKSPACE-SCHEMA-REFERENCE.md','docs/OPERATIONS-AND-TROUBLESHOOTING-RUNBOOK.md','docs/PRODUCTION-CHECKLIST.md']) if(!fs.existsSync(path.join(root,required))) throw new Error(`Required EP-005 operational document missing: ${required}`);
