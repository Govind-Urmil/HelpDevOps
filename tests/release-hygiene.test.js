...
describe('EP-005 release hygiene',()=>{
  it('keeps stale EP wording out of current UI while preserving historical specs',()=>{
    for(const file of ['src/pages/index.astro','src/pages/preflight.astro','src/pages/privacy.astro','src/pages/workspace.astro'])expect(fs.readFileSync(path.join(root,file),'utf8')).not.toMatch(/EP-00[1-4]/);
